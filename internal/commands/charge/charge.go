...
func runList(ctx context.Context, opts *cli.GlobalOpts, args []string) error {
	...
	if opts.Stream {
		...
		// --limit becomes a hard stop only if the user passed it explicitly.
		params.Limit = stripeapi.Int64(100)
		cap := 0
		if cli.LimitExplicit(fs) {
			cap = *limit
		}
		return cli.StreamList(ctx, opts, opts.Client.V1Charges.List(ctx, params), cap)
	}
...
func runSearch(ctx context.Context, opts *cli.GlobalOpts, args []string) error {
	...
	if opts.Stream {
		...
		return cli.StreamSearch(ctx, opts, opts.Client.V1Charges.Search(ctx, params), cap)
	}
	list := opts.Client.V1Charges.Search(ctx, params)
