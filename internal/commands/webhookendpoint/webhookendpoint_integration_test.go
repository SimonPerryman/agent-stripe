...
func TestIntegration_WebhookEndpointList(t *testing.T) {
...
func TestIntegration_WebhookEndpointForEvent(t *testing.T) {
	key := os.Getenv("STRIPE_TEST_KEY")
	if key == "" {
		t.Skip("STRIPE_TEST_KEY not set; skipping integration test")
	}
	...
	go io.Copy(io.Discard, r)
	defer func() { w.Close(); os.Stdout = old }()
	if err := runForEvent(context.Background(), opts, []string{"charge.succeeded"}); err != nil {
